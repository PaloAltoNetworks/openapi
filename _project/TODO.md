...
### 7. Navigation groups were not scoped at all — fixed 2026-09-15
Vrushank pointed the site at `openapi.yaml`, and the rendered nav showed every
tag nested inside *Assistants* — and inside every other group too. Cause:
`docs-navigation.json` scoped each group with `"tag": "Assistants"`, a key that
is not in Mintlify's navigation schema. It was ignored, so each of the 42 groups
autogenerated the whole document. The key came in with the original handoff
(`3a2457e`) and had never been rendered, which is exactly the gap the note above
warns about — the first real preview found it in one look.

Fixed by naming the operations: each group now carries
`"pages": ["POST /chat/completions", ...]`, 187 entries across 42 groups. That
reintroduces method+path as a join key, which the old arrangement was written to
avoid, so `check.py` re-derives the lists from the spec and compares —
negative-tested on a returning `tag` key and on a renamed path, both fire.

This is the second thing about Mintlify's behaviour that only a render could
settle. Worth assuming there is a third: **nothing in this repository can
confirm how the site looks.**
